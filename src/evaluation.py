...
def normalize_label(prediction):
    """Normalize model prediction to standard label format"""
    prediction = normalize_answer(prediction)
    
    # Handle variations of entailment
    if prediction.startswith('entail'):
        return 'entailment'
    # Handle variations of contradiction
    elif prediction.startswith('contradict'):
        return 'contradiction'
    # Handle variations of neutral
    elif prediction.startswith('neutral'):
        return 'neutral'
    else:
        return prediction
...
def esnlive_accuracy(prediction, ground_truth):
    """Calculate accuracy for e-SNLI-VE predictions"""
    # For e-SNLI-VE, there's only one correct answer per sample
    return exact_match_score(prediction, ground_truth)


def eval_accuracies(predictions, ground_truths):
    """
    Calculate accuracy over a set of predictions
    Args:
        predictions: list of model predictions
        ground_truths: list of ground truth labels
    Returns:
        accuracy: overall accuracy
        class_accuracies: dict with per-class accuracies
    """
    ...
    correct = 0
    class_counts = {'entailment': 0, 'contradiction': 0, 'neutral': 0}
    class_correct = {'entailment': 0, 'contradiction': 0, 'neutral': 0}
    
    for pred, truth in zip(predictions, ground_truths):
        norm_pred = normalize_label(pred)
        norm_truth = normalize_label(truth)
        
        # Update class counts
        class_counts[norm_truth] = class_counts.get(norm_truth, 0) + 1
        
        if norm_pred == norm_truth:
            correct += 1
            class_correct[norm_truth] = class_correct.get(norm_truth, 0) + 1
    ...
    for label in class_counts:
        if class_counts[label] > 0:
            class_accuracies[label] = class_correct[label] / class_counts[label]
        else:
