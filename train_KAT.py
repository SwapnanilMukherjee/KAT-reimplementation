...
import src.data
import src.data
from src.data import ESNLIVECollator, load_esnlive_data
import src.model
import os
import json
import pickle
    ...
    while step < opt.total_steps:
        ...
        for i, batch in enumerate(tqdm(train_dataloader)):
            ...
            if step % opt.eval_freq == 0:
                ...
                print("Train loss: {}  Accuracy:{}".format(curr_loss, dev_em))
                model.train()
                if opt.is_main:
...
def evaluate(model, dataset, tokenizer, collator, opt):
    ...
    accuracy, class_accuracies = src.evaluation.eval_accuracies(predictions, ground_truths)
...
if __name__ == "__main__":
    ...
    # Update collator initialization
    collator = src.data.ESNLIVECollator(opt.text_maxlength, tokenizer, answer_maxlength=opt.answer_maxlength)
    ...
    # Update data loading
    train_examples = src.data.load_esnlive_data(
        opt.train_data,
        split_type='train',
        global_rank=opt.global_rank,
        world_size=opt.world_size
    )
    train_dataset = src.data.ESNLIVEDataset(train_examples, opt.n_context)

    # use golbal rank and world size to split the eval set on multiple gpus
    eval_examples = src.data.load_esnlive_data(
    opt.eval_data,
    split_type='test',
    global_rank=opt.global_rank,
    world_size=opt.world_size
    ...
    eval_dataset = src.data.ESNLIVEDataset(eval_examples, opt.n_context)
